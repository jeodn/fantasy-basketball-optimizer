...
def export_z_scores_as_file(final_df: pd.DataFrame, export_types: list[str]) -> None:
    ...
    if "CSV" in export_types:
        json_df.to_csv(DATA_DIR / "fantasy_rankings.csv", index=False)
    ...
    if "JSON" in export_types:
        if not final_df.empty:
            ...
            # Only set index if it's not already the index
            if json_df.index.name != 'player_id':
            ...
            json_df.to_json(DATA_DIR / 'data_zscores.json', orient='index', indent=4)
            print(f"\nData exported to '{DATA_DIR / 'data_zscores.json'}' (keyed by player_id).")
